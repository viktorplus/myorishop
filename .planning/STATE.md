...
| 260912-fct | Recognise the 24 handwritten inventory sheets of `input/12-09-26` (`IMG_7837..IMG_7860`) into the usual receipt file `reports/оприходование-2026-09-12.csv` — 243 rows, 1864 units, 154 codes, 12 shelves (1, 16, 23, 24, 58, 63, 88, 89, 90, 91, 113, 114; all but 23 and 63 new to this project) — plus `reports/отчет-распознавание-2026-09-12.md`. Every sheet total matched the sum of its own breakdown, every one of the 154 codes resolves in the dictionary and carries a `catalog_prices` price, and a dry run of `scripts/import_inventory_receipt.py` parsed all 243 rows with 0 skipped. NOT imported: 52 rows / 340 units are held back for the operator (block A) — the shelf number is scribbled out on `IMG_7848`, `IMG_7853` and the top block of `IMG_7852`; codes `48239` (161 units), `45304` (33), `30960` (9) and the operator's own «19140 ???» are absent from the 12 582-row dictionary; two expiry months are overwritten past reading; six items are written with no code at all. New shorthand «бу» in the expiry column is read as «no expiry» on the evidence of what carries it (wallet, hairbrush, sun visor, makeup sponge, curlers, foot files) — that reading is an assumption, not a fact | 2026-09-12 | bf8c084 | [260912-fct-recognise-inventory-sheets-12-09-26](./quick/260912-fct-recognise-inventory-sheets-12-09-26/) |
