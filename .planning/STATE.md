...
| 260912-fct | Recognise the 24 handwritten inventory sheets of `input/12-09-26` (`IMG_7837..IMG_7860`) into the usual receipt file `reports/оприходование-2026-09-12.csv` — 268 rows, 2113 units, 171 codes, 12 shelves (1, 16, 23, 24, 58, 63, 88, 89, 90, 91, 113, 114; all but 23 and 63 new to this project) — plus `reports/отчет-распознавание-2026-09-12.md`. Every sheet total matched the sum of its own breakdown, and a dry run of `scripts/import_inventory_receipt.py` parsed all 268 rows with 0 skipped, 250 new batches, 13 condition rows and 3 codes without a price. Four operator answers are folded in: warehouse «Офис»; the new shorthand «бу» means «бывшее в употреблении», so it is a CONDITION marker written into the comment column after the shelf (`полка 58, б/у`) and NOT a missing expiry — that makes the importer open a separate batch and forbid a top-up; codes absent from the 12 582-row dictionary (`48239` 161 units, `45304` 33, `30960` 9) are named `Не опознан (код NNNNN)` as in `оприходование-офис-2026-08-31.csv` and carry NULL prices; the scribbled shelf on `IMG_7853` is 63. NOT imported: 26 rows / 90 units still held for the operator — the shelf is unreadable on `IMG_7848` and on the top block of `IMG_7852`, two expiry months are overwritten past reading, six items are written with no code at all, and «3+1» / «2+2» is undeciphered | 2026-09-12 | 07245b9 | [260912-fct-recognise-inventory-sheets-12-09-26](./quick/260912-fct-recognise-inventory-sheets-12-09-26/) |
