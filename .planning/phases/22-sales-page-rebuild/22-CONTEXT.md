...
- **D-11: Fix the mobile batch-card `hx-include` gap in this phase.** Operator confirmed 2026-07-17. `app/templates/mobile_partials/batch_card_picker.html` fires `hx-get` with only `hx-vals` (batch_id/code/row) and no `hx-include`, and htmx only auto-includes the enclosing form on **non-GET** requests — so tapping a batch card drops the accumulated basket (verified empirically: `code_acc` hidden inputs absent from the response). Pre-existing defect, but it sits exactly on D-04's blast radius.

  **Correction to RESEARCH.md Pitfall 3 / UI-SPEC Interaction 10 (verified by hand 2026-07-17):** both claim "every sibling wizard already passes `hx-include="closest form"`; the sale wizard is the outlier." **That is wrong.** The cited lines (`writeoff_step_batch.html:20`, `receipts_step_batch.html:53`, `corrections_step_batch.html:24`) are those wizards' **«Назад» navigation buttons**, not their batch cards. `batch_card_picker.html` is a *shared* partial, so the card tap is missing `hx-include` **uniformly** — the sale wizard is not an outlier.

  **Verified consumer set: THREE wizards, not four** — `sale_step_batch.html:16`, `corrections_step_batch.html:21`, `writeoff_batch_wrap.html:9`. Receipts and Transfers are NOT affected: each ships its own separate picker, and their own template comments say so explicitly (`receipts_step_batch.html:2` "functionally DIFFERENT from batch_card_picker.html"; `transfers_step_batch.html:2` "NOT batch_card_picker.html verbatim"). The pattern mapper's "all four wizards" framing overcounts.

  **Two planning consequences:** (1) the one-attribute change hits Sale + Correction + Write-off at once, so a full `test_mobile_*` regression pass is **mandatory, not precautionary**; (2) the fix starts sending each wizard's own hidden `code`/`name` inputs (e.g. `writeoff_step_batch.html:14-15`) to its `pick_url` — verify each step endpoint tolerates those extra `Query` params rather than assuming a no-op for the siblings.
