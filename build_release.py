...
# Stable product identity for Inno Setup. Generated once (uuid4) and hard-coded
# on purpose: it is what makes every future release UPGRADE the existing
# per-user install instead of registering a SECOND uninstall entry. Without an
# AppId, Inno falls back to AppName, so a rename would silently fork the
# install. NEVER regenerate this — a new AppId is a new product to Windows.
_APP_ID = "1BF2D689-291E-4E44-B502-BC4EAEBE4C32"
...
def generate_iss(*, dist_dir: Path, version: str, dest: Path) -> Path:
    ...
    ``dist_dir`` is ENFORCED, not decorative: the ``[Files] Source:`` paths are
    relative to the .iss file's own directory (Inno's ``SourceDir`` default), so
    correctness silently depends on ``dest.parent == dist_dir``. Passing a
    ``dist_dir`` that differs from ``dest.parent`` used to emit a perfectly
    normal script pointing at the wrong tree; it now raises ``ValueError``.
    """
    dest = Path(dest)
...
[Setup]
; AppId identifies the product across renames. Without it Inno falls back to
; AppName, so any future rename would create a SECOND uninstall entry instead of
; upgrading in place. Never change this GUID.
AppId={{{{{_APP_ID}}}
AppName=MyOriShop
AppVersion={version}
...
; ignoreversion: Inno's default compares version resources and SKIPS a file whose
; installed copy is equal-or-newer — a repair/reinstall over an install whose app\\
; was already self-updated would otherwise keep stale versioned binaries.
...
[UninstallDelete]
; Swap residue the LAUNCHER creates at runtime — Inno removes only what it
; installed, so without this an uninstall leaves up to three ~30 MB bundle copies
; behind (app.failed\\ is retained by design until the next rollback rotates it,
; i.e. forever on a healthy install). data\\ is deliberately NOT listed: it is the
; operator's DB, backups and identity.
