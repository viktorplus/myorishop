...
def _origin_business_day(origin: Operation) -> str:
    ...
    Mirrors app.routes.returns._origin_business_day verbatim (D-21: mobile
    mirrors desktop, it does not diverge) — the origin sale's BUSINESS date,
    falling back to the created_at day for a pre-0027 row whose business_date is
    still NULL (DATE-08). The result is a 10-char ISO date and MUST be rendered
    with `| ru_date`, never `| local_dt`.
    """
...
def _origin_context(
    ...
    return {
        ...
        "errors": errors,
        # DATE-01: the flat echo key return_confirm.html reads. Unlike the sale
        # wizard, this screen IS re-rendered on a 422 (whole-block outerHTML
        # swap), so the typed date has to be echoed or it would be lost.
        "op_date": op_date,
        "saved": None,
    }
...
def mobile_return_create(
    ...
    context = {
        ...
        "origin_batch": resolve_return_batch(session, origin),
        "errors": {},
        # The success screen has no form to redisplay; the echo is 422-only.
        "op_date": "",
